...
class MA_trader(object):
    ...
    def __init__(self):
        ...
        self.angle_ma_18 = 99
        self.angle_ma_42 = 99
        self.kline_dimemsion = "5m"
        self.slope_offset = 50
    ...
    def run(self):
        ...
        try:
            ...
            while not fc.stop_singal_from_client:
                ...
                msg = str(self.henged_run_time) + ',【目前盈利】:' + str(self.profit_total) + ', 上次亏损：' + str(self.last_time_profit) + ', 当前操作数量：' + str(self.quantity) + ', ' + (
                            '， 最新的压力线：' + str(press_price) + ', 最新的支撑线：' + str(sustain_price)) if (
                            float(press_price) != 0 or float(sustain_price) != 0) else ""
                # print(msg)

                loop_count = loop_count + 1

                if loop_count * self.pause_time % 60 == 15:
                ...
                if position_info_long == -1 or position_info_short == -1:
                ...
                #压力线
                # press_price = index.calc_press(config.symbol, self.kline_dimemsion, self.demical_length, ma_number_42, time.time())

                #支撑线
                # sustain_price = index.calc_sustain(config.symbol, self.kline_dimemsion, self.demical_length, ma_number_42, time.time())
                ...
                #均线
                ma_price_1 = index.calcSlopeMA(config.symbol, self.kline_dimemsion, self.demical_length, ma_number_1, self.slope_offset)
                #30分钟均线
                ma_price_30 = index.calcSlopeMA(config.symbol, self.kline_dimemsion, self.demical_length, ma_number_30, self.slope_offset)

                print('ma_price_1:' + str(ma_price_1) + ', ma_price_30:' + str(ma_price_30))
                ma_price_35 = ''#index.calcSlopeMA(config.symbol, self.kline_dimemsion, self.demical_length, ma_number_35, self.slope_offset)

                if not ma_pre_price_1:
                    ma_pre_price_1 = ma_price_1[1]
                ...
                self.deal_with_line("", ma_price_30[1], ma_pre_price_1, ma_price_1[1], position_info_long, position_info_short, ma_price_35, self.quantity)
                    ...
                    # self.deal_with_line("press", press_price, ma_pre_price_1, ma_price_1[1], position_info_long, position_info_short, ma_price_35, self.quantity)
                ...
                if ma_price_1[1] != ma_price_30[1]:
                    ma_pre_price_1 = ma_price_1[1]
                # print('替换ma_pre_price:' + str(ma_pre_price_1))

                sleep_time = self.pause_time
                time.sleep(sleep_time)
                # print('ok,  ' + 'sleep ' + str(sleep_time) + ' secs')
            ...
            try:
                if float(position_info_long[0]) == 0.0 and float(position_info_short[0]) == 0.0:
                    pass
                else:
    ...
    def position_check_and_clear(self, tag_line, ma_price_3, press_price, sustain_price, position_info_long, position_info_short):
        print('ma_price_3:' + str(ma_price_3))
        if tag_line == 'press' and ma_price_3 > press_price and float(position_info_short[0]) != 0.0:
            msg = 'ma3价格涨破压力线了，空单还在，要清掉，不然很危险'
            short_loss = float(position_info_short[2]) - float(position_info_short[1]) * float(
                self.quantity) * 0.0008  # 记录亏的
            if float(position_info_short[2]) < 0.0:
                self.current_loss_profit += short_loss
            self.last_time_profit = short_loss
            if self.last_time_profit < 0:
                self.fail_times += 1
            else:
                self.fail_times = 0
            self.close_short(round(self.quantity, 1))
            ...
            msg = 'ma3价格跌破支撑线了，多单还在，要清掉，不然很危险'
            long_loss = float(position_info_long[2]) - float(position_info_long[1]) * float(
                self.quantity) * 0.0008  # 记录亏的
            if float(position_info_long[2]) < 0.0:
                self.current_loss_profit += long_loss
            self.last_time_profit = long_loss
            if self.last_time_profit < 0:
                self.fail_times += 1
            else:
                self.fail_times = 0
    ...
    def set_quantity(self, quantity):
        ...
        # 挽救震荡磨损的办法，上次如果亏损了，这次就加倍，一份是为了覆盖上次亏损之后止盈出场的，但出场后，剩下的（比如某次双份是8倍，止盈后应该降为1倍）应该缩减为1倍；控制风险
        # 上次是盈利的，继续保持，不加倍
        # 亏损为正之后，恢复原来的份额
        if self.current_loss_profit < 0.0 and self.last_time_profit < 0.0:
            self.quantity = round(float(config.quantity) * (round((float(quantity) / float(config.quantity)) + 0.2, 1)), 1)  # 翻倍成本太高了，每次递增一倍吧
    ...
    def deal_with_line(self, tag_line, line_price, ma_pre_price, ma_price, position_info_long, position_info_short, cooperate_ma_price, quantity):
        ...
        long_position_price = position_info_long[1]
        short_position_price = position_info_short[1]
        ...
        if not position_info_long and not position_info_long[0] and not position_info_long[1] and not position_info_long[2] and not position_info_long[3]:
            long_position_amt = position_info_long[0]
            long_position_price = position_info_long[1]
            position_info_long_profit = position_info_long[2]
            position_info_long_initial_margin = position_info_long[3]
        if not position_info_short and not position_info_short[0] and not position_info_short[1] and not position_info_short[2] and not position_info_short[3]:
            short_position_amt = position_info_short[0]
            short_position_price = position_info_short[1]
            position_info_short_profit = position_info_short[2]
            position_info_short_initial_margin = position_info_short[3]
        ...
        # tag_line == 'press' and
        if ma_price > line_price * 1.002:
            # print('2222:')
            # if ma_price > line_price:
            if ma_pre_price > line_price * 1.002:
                # print('3333:')
                print('这次（' + str(ma_price) + '）和上次(' + str(ma_pre_price) + ')的ma都在ma30(' + str(line_price) + ')* 高0.2%的上方')
                self.saveLastTimeLoss(quantity, long_position_amt, short_position_amt, long_position_price, short_position_price, position_info_long_profit, position_info_short_profit, position_info_long_initial_margin, position_info_short_initial_margin)
                # 压力线之上还有空单，就要清掉；支撑线之下还有多单，也要清掉，那如果是用来抄底的呢？？,而且可能ma还没穿过压力线
                time.sleep(3)
                self.position_check_and_clear(ma_price, tag_line, line_price, line_price, position_info_long, position_info_short)
                # print('6666:')
            elif ma_pre_price <= line_price * 1.002:
                # print('4444:')
                print('ma1涨穿ma30高0.2%的地方了')
                if float(short_position_amt) != 0.0:
                    short_loss = float(position_info_short_profit) - float(short_position_price) * float(quantity) * 0.0008 # 记录亏的
                    if float(position_info_short_profit) < 0.0:
                        self.current_loss_profit = self.current_loss_profit + short_loss
                    self.last_time_profit = short_loss
                    if self.last_time_profit < 0:
                        self.fail_times += 1
                    else:
                    ...
                    print(msg)
                    Message.dingding_warn(msg)
                    if self.fail_times >= 10000000:
                        Message.dingding_warn('操作亏本超过3次，说明现在可能是震荡行情，开单之前，休息一下:' + str(10 * self.pause_time) + '秒')
                        sleep_time = 10 * self.pause_time
                        time.sleep(sleep_time)
                        self.fail_times = 0
                        return
                if float(long_position_amt) == 0.0:
                    self.set_quantity(quantity)  # 亏了下次加倍
                    self.open_long(round(float(self.quantity), 1))
                    # tag_line == 'sustain' and
        elif ma_price < line_price * 0.998:
        #elif ma_price < line_price:
            if ma_pre_price < line_price * 0.998:
                print('这次（' + str(ma_price) + '）和上次(' + str(ma_pre_price) + ')的ma都在ma30(' + str(line_price) + ')* 0.998下方')
                self.saveLastTimeLoss(quantity, long_position_amt, short_position_amt, long_position_price, short_position_price, position_info_long_profit, position_info_short_profit, position_info_long_initial_margin, position_info_short_initial_margin)
                time.sleep(3)
                self.position_check_and_clear(ma_price, tag_line, line_price, line_price, position_info_long, position_info_short)
            elif ma_pre_price >= line_price * 0.998:
                # print('5555:')
                print('ma1跌穿ma30了' + str(long_position_amt) +', ' + str(position_info_long_profit))
                if float(long_position_amt) != 0.0:
                    long_loss = float(position_info_long_profit) - float(long_position_price) * float(quantity) * 0.0008 # 记录亏的
                    if float(position_info_long_profit) < 0.0:
                        self.current_loss_profit = self.current_loss_profit + long_loss
                    self.last_time_profit = long_loss
                    if self.last_time_profit < 0:
                        self.fail_times += 1
                    else:
    ...
    def saveLastTimeLoss(self, quantity, long_position_amt, short_position_amt, long_position_price, short_position_price, position_info_long_profit, position_info_short_profit, position_info_long_initial_margin, position_info_short_initial_margin):
        '''
        ...
        msg = ''
        # if self.angle_ma_42 <= self.smooth_line_angle * 1.6:# 较为平缓时，再操作
        save_profit_quantity = round(float(quantity) * ((int(float(quantity) / float(config.quantity)) - 1) / (float(quantity))), 1) if float(quantity) > 1 else 0
        ...
        print('当前多单浮动盈亏:' + str(position_info_long_profit))
        print("补仓的多单当前的浮动盈亏:" + str(save_share_current_profit_long))
        print('当前空单浮动盈亏:' + str(position_info_short_profit))
        print("补仓的空单当前的浮动盈亏:" + str(save_share_current_profit_short))
        print("已产生的亏损:" + str(self.current_loss_profit))
        ...
        if not float(position_info_long_initial_margin) == 0.0 and not float(self.quantity) == 0.0:
            print("多单目前的杠杆倍数:" + str((float(long_position_price) / (float(position_info_long_initial_margin) / float(self.quantity)))))
        if not float(position_info_short_initial_margin) == 0.0 and not float(self.quantity) == 0.0:
            print("空单目前的杠杆倍数:" + str((float(short_position_price) / (float(position_info_short_initial_margin) / float(self.quantity)))))
        if float(short_position_amt) == 0.0 and float(position_info_long_profit) > 0 and (save_share_current_profit_long > max(abs(self.current_loss_profit), abs(self.profit_total) if self.profit_total < 0 else 0)): # 0.005 为手续费
            ...
            Message.dingding_warn(msg)
        if self.has_earn_handling_charge_long is False and float(short_position_amt) == 0.0 and float(position_info_long_profit) > 0 and float(position_info_long_profit) > handling_charge_long * 10: # 0.005 为手续费
            self.has_earn_handling_charge_long = True
            msg = '本次多单盈利: ' + str(position_info_long_profit) + ' 覆盖本次和上次的手续费( ' + str(handling_charge_long * 10) + ' )了，减仓! 原仓: ' + str(quantity) + ', 减的仓：' + str(quantity * 0.2) + ', 减了之后的： ' + str(quantity * 0.8)
            self.close_long(round(quantity * 0.2, 1))  # 平多
            self.quantity = round(quantity * 0.8, 1) # 减了之后的
            self.profit_total -= handling_charge_long * 10
            msg += ', 总盈利：' + str(self.profit_total)
            print(msg)
            print('多单手续费：' + str(handling_charge_long))
            Message.dingding_warn(msg)
        if float(long_position_amt) == 0.0 and float(position_info_short_profit) > 0 and save_share_current_profit_short > max(abs(self.current_loss_profit), abs(self.profit_total) if self.profit_total < 0 else 0): # 0.005 为手续费
            ...
            Message.dingding_warn(msg)
        if self.has_earn_handling_charge_short is False and float(long_position_amt) == 0.0 and float(position_info_short_profit) > 0 and float(position_info_short_profit) > handling_charge_short * 10: # 0.005 为手续费
            self.has_earn_handling_charge_short  = True
            msg = '本次空单盈利: ' + str(position_info_short_profit) + ' 覆盖本次和上次的手续费( ' + str(handling_charge_short * 10) + ' )了，减仓! 原仓: ' + str(quantity) + ', 减的仓：' + str(quantity * 0.2) + ', 减了之后的： ' + str(quantity * 0.8)
            self.close_short(round(quantity * 0.2, 1))  # 平多
            self.quantity = round(quantity * 0.8, 1) # 减了之后的
            self.profit_total -= handling_charge_short * 10
            msg += ', 总盈利：' + str(self.profit_total)
            print(msg)
        ...
        # #部分止盈
        #
        # if self.has_earn_target_long is False and float(position_info_long_initial_margin) != 0.0 and float(position_info_long_profit) > 0 and float(position_info_long_profit) / float(position_info_long_initial_margin) >= 0.004 * (float(long_position_price) / (float(position_info_long_initial_margin) / float(self.quantity))):
        #     self.has_earn_target_long = True
        #     self.profit_total += float(position_info_long_profit) * 0.5
        #     self.close_long(round(float(quantity * 0.5), 1))
        #     self.current_loss_profit -= float(position_info_long_profit) * 0.5
        #     self.last_time_profit = 0
        #     msg = '在' + str(self.current_price) + ', 多单利润达到目标了，减为半仓！总盈利：' + str(self.profit_total) + '， 剩余仓位：' + str(round(float(quantity * 0.5), 1))
        #     self.quantity = round(float(quantity * 0.5), 1)
        #     print(msg)
        #     Message.dingding_warn(msg)
        #
        # if self.has_earn_target_short is False and float(position_info_short_initial_margin) != 0.0 and float(position_info_short_profit) > 0 and float(position_info_short_profit) / float(position_info_short_initial_margin) >= 0.006 * abs(float(short_position_price) / (float(position_info_short_initial_margin) / float(self.quantity))):
        #     self.has_earn_target_short = True
        #     self.profit_total += float(position_info_short_profit) * 0.5
        #     self.close_short(round(float(quantity * 0.5), 1))
        #     self.current_loss_profit -= float(position_info_short_profit) * 0.5
        #     self.last_time_profit = 0
        #     self.quantity = round(float(quantity * 0.5), 1)
        #     msg = '在' + str(self.current_price) + ', 空单利润达到目标了，减为半仓！总盈利：' + str(self.profit_total) + '， 剩余仓位：' + str(round(float(quantity * 0.5), 1))
        #     print(msg)
        #     Message.dingding_warn(msg)
        #
        # #先简单处理，二次止盈
        ...
        #     msg = '在' + str(self.current_price) + ', 空单三次利润达到目标了，清仓！总盈利：' + str(self.profit_total) + '， 剩余仓位：' + str(round(float(quantity * 0.5), 1))
        #     print(msg)
        #     Message.dingding_warn(msg)
    ...
    def open_long(self, quantity):
        ...
        if spot_res and spot_res['orderId']:
            print("开多单成功")
            Message.dingding_warn(str(self.current_price) + "买入一份多单了！")
            # self.set_spot_ratio()
            # self.set_spot_next_sell_price(float(self.cur_market_future_price))
    ...
    def close_long(self, quantity):
        ...
        if spot_res and spot_res['orderId']:
            ...
            Message.dingding_warn(str(self.current_price) + "平掉一份多单了！")
            # msg = '多单卖出获利了！获得：' + str(
    ...
    def open_short(self, quantity):
        ...
        if future_res and future_res['orderId']:
            print("开空单成功")
            Message.dingding_warn(str(self.current_price) + "买入一份空单了！")
    ...
    def close_short(self, quantity):
        ...
        future_res = self.http_client_future.place_order(config.symbol, OrderSide.BUY, "SHORT", order_type, quantity,
                                                         'none', time_inforce)
        if future_res and future_res['orderId']:
            Message.dingding_warn(str(self.current_price) + "平掉一份空单了！")
            # self.set_future_ratio()
            # self.set_future_next_buy_price(float(self.cur_market_future_price))

...
class HengedGrid(object):
    ...
    def run(self):
        ...
        self.cur_market_future_price = self.http_client_spot.get_latest_price(config.symbol)#self.http_client_future.get_latest_price(config.symbol).get('price')
        # 设定精度，无所谓现货或者合约
        ...
        #贷出功能
        self.http_client_spot.submit_lending_offers(config.coin, 0, 0.000016)
        ...
        self.diff_time = time.time() - begin_time + int(dynamicConfig.get_trade_benefit()[config.symbol.replace('/', '')][0])
        ...
        # for i in range(6, len(self.rows)):
            ...
            try:
                ...
                if fc.position_side == 'LONG':
                    isTrendComing = False #index.calcTrend_MK(symbol_to_check_trend, "60", ascending, self.demical_length) # 因为可能会导致请求过于频繁而出错
                elif fc.position_side == 'SHORT':
                    isTrendComing = False #index.calcTrend_MK(symbol_to_check_trend, "60", descending, self.demical_length)
                else:
                    isTrendComing = False
                ...
                #要卖出时，市场价也要大于最近上次那个的价格，因为计算盈利的时候，要拿上次的价格来算盈利的，如果max(sell_price,market_price) < get_last_spot_price,会亏钱 # 可能高位的单需要留着，因为还没到它的目标止盈点
                    ...
                    spot_res = self.close_long(time_format, False, str(self.cur_market_future_price))
                ...
                #要买回时，市场价也要小于最近上次那个的价格，因为计算盈利的时候，要拿上次的价格来算盈利的，如果min(buy_price,market_price) > get_last_future_price, 会亏钱 # 可能高位的单需要留着，因为还没到它的目标止盈点
                    ...
                    future_res = self.close_short(time_format, False, str(self.cur_market_future_price))
    ...
    def close_long(self, time_format, cut_position = False, price='none', tickOutBottom = False):
        ...
        if self.spot_step > 0:
            ...
            if price != 'none':
                ...
                time_inforce = "GTC"
            spot_res = self.http_client_spot.place_order(config.symbol, OrderSide.SELL.value, 'LONG', OrderType.LIMIT.value, self.quantity, round(float(self.cur_market_future_price), 2))
    ...
    def open_short(self, time_format):
        ...
        future_res = self.http_client_future.place_order(config.symbol, OrderSide.SELL.value, "SHORT", OrderType.LIMIT.value, self.quantity, round(float(self.cur_market_future_price), 2))
    ...
    def close_short(self, time_format, cut_position=False, price='none'):
        ...
        if self.future_step > 0:
            ...
            if price != 'none':
                order_type = OrderType.LIMIT
                time_inforce = "GTC"
            future_res = self.http_client_future.place_order(config.symbol, OrderSide.BUY.value, "SHORT", order_type.value, self.quantity, price)
    ...
    def set_spot_ratio(self):
        ...
        ratio_24hr = round(float(self.http_client_spot.get_ticker_24hour(config.symbol)), 1)
        if abs(ratio_24hr) > 6:
                print("24小时上涨或下跌趋势")
    ...
    def open_receiver(self):
        ...
        app.run(host='104.225.143.245', port=5000 if config.platform == 'binance_future' else 5006, threaded=True)
    ...
    def init_record_price_list(self):
        ...
        with open('../data/trade_info_%s.json' % config.symbol.replace('/', ''), 'r') as df:
    ...
    def save_trade_info(self):
        ...
        with open('../data/trade_info_%s.json' % config.symbol.replace('/', ''), "w") as df:
    ...
    def close_previous_position(self, time_format):
        ...
        for tmp in dynamicConfig.record_spot_price:
            self.cur_market_future_price = self.http_client_spot.get_latest_price(config.symbol)

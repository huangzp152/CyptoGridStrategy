...
class HengedGrid(object):
    ...
    def run(self):
        ...
        with open(kline_path, 'r', encoding='utf-8') as df:
            ...
            self.set_ratio()
            self.set_spot_price(float(self.cur_market_price))
            ...
            for i in range(6, len(self.rows)):
                ...
                #开多单（买入持仓） 趋势上升时不买
                #多单市场价要低于你的买入价，才能成交
                elif float(self.cur_market_price) <= float(self.spot_buy_price) and not index.calcTrend_MK(config.symbol, "5m", ascending, self.demical_length, i):
                    #test
                    ...
                    if spot_res['orderId']:
                        print("开多单成功")
                        self.decreaseMoney(float(self.cur_market_price) * float(config.quantity))
                ...
                #平掉多单（卖出获利）趋势上升时不卖
                #多单市场价要高于你的卖出价，才能成交
                elif float(self.cur_market_price) >= float(self.spot_sell_price) and float(self.cur_market_price) > float(self.get_last_spot_price()) and not index.calcTrend_MK(config.symbol, "5m", ascending, self.demical_length, i):
                    ...
                    if self.spot_step > 0:
                        ...
                        if spot_res['orderId']:
                            ...
                            self.addMoney(float(self.cur_market_price) * float(self.quantity))
                            self.set_ratio()
                ...
                #开空单（卖出借仓），趋势下跌时不买
                #空单市场价要高于你的卖出价，才能成交
                elif float(self.cur_market_price) >= float(self.future_sell_price) and not index.calcTrend_MK(config.symbol, "5m", descending, self.demical_length, i):
                    ...
                    if future_res['orderId']:
                        print("开空单成功")
                        self.addMoney(float(self.cur_market_price) * float(config.quantity))
                        dynamicConfig.total_invest += float(self.cur_market_price) * float(config.quantity)
                ...
                #平掉空单（买入获利）下跌趋势时不买
                #空单市场价要低于你的买回价，才能成交
                elif float(self.cur_market_price) <= float(self.future_buy_price) and float(self.cur_market_price) < float(self.get_last_future_price()) and not index.calcTrend_MK(config.symbol, "5m", descending, self.demical_length, i):
                    ...
                    if self.future_step > 0:
                        ...
                        dynamicConfig.order_list.append(future_res)
                        if future_res['orderId']:
                            self.decreaseMoney(float(self.cur_market_price) * float(self.quantity))

...
class HengedGrid(object):
        ...
        if config.platform == 'test':
            ...
            for m in range(0, len(symbol_list)):
                ...
                for k in range(-5, -15, -1):
                    ...
                    print("kline_path url: " + str(kline_path))
        ...
        #test
            ...
            for l in range(0, len(symbol_list)):
                ...
                for j in range(0, len(martin_list)):# 前多少为马丁
                    ...
                    if config.platform == 'test':
                        fc.end_martin_grid = martin_list[j]
    ...
    def loop_one_ratio(self):
        ...
        self.current_all_spot_quantity = float(self.quantity)
        self.current_all_future_quantity = float(self.quantity)
        # 设定仓位
        ...
        # if self.spot_step != int(float(self.http_client_spot.get_future_position_info(config.symbol)) / float(float(self.quantity))):
        ...
        print('check quantity: ' + str(float(self.quantity)))
        ...
        # future_res = self.http_client_future.place_order('BTCUSDT', OrderSide.SELL, OrderType.MARKET, float(self.quantity), round(float(self.cur_market_future_price), 2), "")
        # spot_res = self.http_client_spot.place_order('BTCUSDT', OrderSide.BUY, OrderType.MARKET, quantity=float(self.quantity), price=round(float(self.cur_market_spot_price), 2), time_inforce="")
        ...
        while not fc.stop_singal_from_client or (True if (config.platform == 'test' and loop_count < len(self.rows[config.symbol])) else False):
            ...
            try:
                ...
                # 开仓资金占总资金1/3以上走马丁
                # occupied_share = sum([float(dynamicConfig.record_spot_price[i]) * 1.01 * (i + 1) * float(self.quantity) for i in range(0, len(dynamicConfig.record_spot_price))])
                ...
                msg8 = ''
                if self.grid_side == 'LONG':
                    msg7 = "下一份多单买入价：" + str(self.spot_buy_price) + "， 数量：" + str(float(self.quantity) * (pow(self.martin_add_ratio,
                                                                                                       len(dynamicConfig.record_spot_price) + 1) if isMartin else 1)) + "，这份【多单卖出价】" + str(
                        tag) + "：" + str(
                        self.spot_sell_price) + "， 数量：" + str(float(self.quantity) * (
                        sum([pow(self.martin_add_ratio, i + 1) for i in range(1, len(dynamicConfig.record_spot_price))]) if (
                ...
                # float(self.quantity) = self._format(quantity_basic)  # 买的不一定是0.0004,应该是现在的市场价买10u的份额
                # spot_res = None
                # future_res = None
    ...
    def nearly_full_position(self):
        return False
        # if float(((sum(dynamicConfig.record_spot_price) if len(dynamicConfig.record_spot_price) > 0 else 0) + float(sum(dynamicConfig.record_future_price) if len(dynamicConfig.record_future_price) > 0 else 0)) * float(self.quantity)) / (float(self.spot_money) * int(self.leverage)) >= 0.95:
    ...
    def open_long(self, time_format, build_position_share = False):
        ...
        isMartin = True if len(dynamicConfig.record_spot_price) <= self.end_martin_grid else False
        quantity = max(round((10.01 / float(self.cur_market_future_price)), 3), (round(float(self.quantity) * pow(self.martin_add_ratio, len(dynamicConfig.record_spot_price)+1) if (isMartin and not build_position_share) else float(self.quantity), 3)))# uni精度为整数
    ...
    def close_long(self, time_format, cut_position = False, price='none', tickOutBottom = False):
        ...
        if self.spot_step > (0 if tickOutBottom else (1 if isMartin else 0)):
            ...
            tag = '【马丁】' if isMartin else '【网格】'
            if len(dynamicConfig.record_spot_price) <= 1:
                print("len(dynamicConfig.record_spot_price):" + str(len(dynamicConfig.record_spot_price)))
                current_average_spot_price = float(dynamicConfig.record_spot_price[0])
                print("current_average_spot_price:" + str(current_average_spot_price))
            else:
                print("len(dynamicConfig.record_spot_price):" + str(len(dynamicConfig.record_spot_price)))
                current_average_spot_price = round(sum([float(dynamicConfig.record_spot_price[i]) * pow(self.martin_add_ratio, i+1) for i in range(0, len(dynamicConfig.record_spot_price))]) / (self.martin_add_ratio * (1 - pow(self.martin_add_ratio, len(dynamicConfig.record_spot_price))) / (1 - self.martin_add_ratio)) if self.martin_add_ratio > 1 else sum([float(dynamicConfig.record_spot_price[i]) for i in range(0, len(dynamicConfig.record_spot_price))]) / len(dynamicConfig.record_spot_price), 3)
                print("current_average_spot_price:" + str(current_average_spot_price))
            ...
            spot_res = {'orderId':'backtestid'} if config.platform == 'test' else self.http_client_spot.place_order(config.symbol, OrderSide.SELL, order_type, float(self.quantity) if tickOutBottom is True else float(spot_quantity), price=str(price))
            ...
            if spot_res and 'orderId' in spot_res.keys():
                ...
                msg = tag + '【' + str(config.symbol) + '】多单卖出获利了！获得：' + str(round((float(self.cur_market_future_price) - open_spot_price) * float(spot_quantity), 2)) + "， 卖出价格：" + str(self.cur_market_future_price) + ", 买入的价格:" + str(open_spot_price
                    ) + ", 买入的数量：" + str(spot_quantity) + ', 目前总获利：' + str(round(dynamicConfig.total_earn, 2)) + ', 总格子数：' + str(int(dynamicConfig.total_earn_grids)) + ', 利润率：' + self.gross_profit + ', 多仓:' + str(self.spot_step) + '， 仓位均价(卖出前)：' + str(current_average_spot_price) + ', 空仓:' + str(self.future_step) + ', 仓位具体信息, 多仓:' + str(dynamicConfig.record_spot_price) + ', 空仓:' + str(dynamicConfig.record_future_price) + ', 底仓：' + str(dynamicConfig.long_bottom_position_price) + ', (' + str(self.get_long_bottom_position_scale()) + '), 阈值：' + str(fc.long_bottom_position_share) + ', ' + self.grid_run_time + ', ' + self.last_get_profit_time_delta
                print(msg)
                Message.dingding_warn(msg)
    ...
    def open_short(self, time_format):
        ...
        quantity = max(round((10.01 / float(self.cur_market_future_price)), 3), (float(self.quantity) * pow(self.martin_add_ratio, len(dynamicConfig.record_future_price) + 1) if isMartin else float(self.quantity)))
    ...
    def close_short(self, time_format, cut_position=False, price='none'):
        ...
        if self.future_step > (1 if isMartin else 0):
            ...
            self.current_all_spot_quantity = float(self.quantity) * (self.martin_add_ratio * (1 - pow(self.martin_add_ratio, len(dynamicConfig.record_future_price) - 1)) / (1 - self.martin_add_ratio) if ((len(dynamicConfig.record_future_price) > 1) and isMartin) else 1)
            future_quantity = self.current_all_future_quantity if isMartin else float(self.quantity)
    ...
    def add_total_invest(self, value):
        dynamicConfig.total_invest = value
        print('total_invest:' + str(dynamicConfig.total_invest))
        #dynamicConfig.record_spot_price.sort(reverse=True)#降序
        self.save_trade_info()
    ...
    def get_spot_share(self):
        ...
        dynamicConfig.spot_step = int(len(dynamicConfig.record_spot_price)) #int(float(self.http_client_spot.get_spot_position_info(config.coin)) / float(float(self.quantity))) # 取该货币的仓位，处以每份的数量，结果为份数
        #测试时默认是0开始
        self.spot_step = dynamicConfig.spot_step
        # print("get_spot_share, dynamicConfig.spot_step:" + str(dynamicConfig.spot_step), ", position:" + self.http_client_spot.get_future_position_info(config.symbol) + ", every piece quantity:" + float(self.quantity))
    ...
    def set_future_next_buy_price(self, deal_price):
        ...
        if self.end_martin_grid > 0 and len(dynamicConfig.record_future_price) > 0 and len(dynamicConfig.record_future_price) <= self.end_martin_grid:
            ...
            # self.current_all_future_quantity = float(self.quantity) * (len(dynamicConfig.record_future_price) - 1)  # 留一份
        else:
    ...
    def get_future_share(self):
        ...
        #         dynamicConfig.future_step = len(dynamicConfig.record_future_price) #abs(int(float(item['positionAmt']) / float(float(self.quantity))))
        #         self.future_step = dynamicConfig.future_step
        #         print(f"positionSide:{item['positionSide']}")
        #         print("get_future_share, dynamicConfig.future_step:" + str(dynamicConfig.future_step),
        #               ", position:" + item['positionAmt'] + ",every piece quantity:" + float(self.quantity))
        #         return self.future_step
        #         #测试时默认是0开始
        # return 0
    ...
    def init_record_price_list(self):
        ...
        with open(trade_info_file_path, 'r') as df:
            ...
            if 'record_martin_grids' in record_price_dict_to_file.keys():
                ...
                dynamicConfig.record_martin_grids = float(record_price_dict_to_file['record_martin_grids'])
            else:
                self.save_part_of_trade_info(record_price_dict_to_file, 'record_martin_grids', dynamicConfig.record_martin_grids)

            if 'total_invest' in record_price_dict_to_file.keys():
                print(f"record_price_dict_to_file['total_invest']:{record_price_dict_to_file['total_invest']}")
                dynamicConfig.total_invest = float(record_price_dict_to_file['total_invest'])
            else:
                self.save_part_of_trade_info(record_price_dict_to_file, 'total_invest', dynamicConfig.total_invest)
    ...
    def save_trade_info(self):
        print("存储记录的价格到文件里")
        print(f"save_trade_info, record_spot_price:{dynamicConfig.record_spot_price}, record_future_price:{dynamicConfig.record_future_price}, dynamicConfig.long_bottom_position_price:{dynamicConfig.long_bottom_position_price}, dynamicConfig.record_martin_grids:{dynamicConfig.record_martin_grids}, 'total_invest':{dynamicConfig.total_invest}")
        record_price_dict_to_file = {'record_spot_price':dynamicConfig.record_spot_price, 'record_future_price':dynamicConfig.record_future_price, 'long_bottom_position_price':dynamicConfig.long_bottom_position_price, 'record_martin_grids':str(dynamicConfig.record_martin_grids), 'total_invest':str(dynamicConfig.total_invest)}

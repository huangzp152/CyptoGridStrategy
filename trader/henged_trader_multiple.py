...
class HengedGrid(object):
    ...
    def __init__(self, cfg, market, product):
        ...
        if market.platform == 'ftx':
            self.http_client_spot = ftx_future(api_key=api_key, api_secret=api_secret)
            self.http_client_future = ftx_future(api_key=api_key, api_secret=api_secret)
    ...
    def getMoney(self):
        ...
        if res:
            if self.market == 'binance':
                totalAccountValue = res.get('totalAccountValue')
                print("totalAccountValue:" + str(totalAccountValue))
                return totalAccountValue
            elif self.market == 'ftx':
                assets = res.get('assets')
                for asset in assets:
                    if self.product.symbol.endswith(asset.get('asset')):
                        return asset.get('crossWalletBalance')
        return 0

    def getAsset(self):
        ret = self.http_client_future.get_future_asset(self.product.coin)
        # print('ret:' + str(ret))
        return ret

    def set_leverage(self, leverage):
        self.http_client_future.set_future_leverage(self.product.symbol, leverage)

    def addMoney(self, money):
        res = float(self.getMoney()) + float(money)
        with open('/home/code/binance/data/test_account_%s.txt' % self.product.symbol, 'w', encoding='utf-8') as df:
            df.write(str(res))
        pass
    ...
    def run(self, product):
        ...
        # self.cur_market_spot_price = self.http_client_spot.get_latest_price(self.product.symbol).get('price')
        self.cur_market_future_price = self.http_client_spot.get_latest_price(self.product.symbol)#self.http_client_future.get_latest_price(self.product.symbol).get('price')
        # 设定精度，无所谓现货或者合约
        ...
        self.quantity = self.format(quantity_basic)
        #贷出功能
        if self.market == 'ftx':
            self.http_client_spot.submit_lending_offers(self.product.coin, 0, 0.000016)
            result = self.http_client_spot.get_lending_offers()
            print('get_lending_offers result:' + str(result))
        # 设定仓位
        # self.spot_step = self.get_spot_share() #现货仓位 #self.get_step_by_position(True) #  合约
        # self.future_step = self.get_future_share() #self.get_step_by_position(False) 一样的
        self.set_spot_share(len(self.record_spot_price))
        self.set_future_step(len(self.record_future_price))

        # if self.spot_step != int(float(self.http_client_spot.get_future_position_info(self.product.symbol)) / float(self.quantity)):
        ...
        # test check value
        # print('check account exchangeinfo: ' + str(self.http_client_spot.exchangeInfo(self.product.symbol)))  # 保留账户模拟数据
        # print('check account assets spot: ' + str(self.http_client_spot.get_future_position_info(self.product.symbol)))  # 保留账户模拟数据
        # print('check account assets future: ' + str(self.http_client_future.get_future_asset(self.product.symbol)))
        # print('check account: ' + str(self.http_client_spot.get_account_info(self.product.symbol)))# 查询现货指定货币的仓位
        ...
        self.set_spot_next_buy_price(float(self.cur_market_future_price))# if len(self.record_spot_price) == 0 else float(self.record_spot_price[-1]))
        self.set_spot_next_sell_price(float(self.cur_market_future_price))
        self.future_step = self.future_step
        self.set_future_next_sell_price(float(self.cur_market_future_price))# if len(self.record_future_price) == 0 else float(self.record_future_price[-1]))
        ...
        begin_time = time.time() # int(self.get_trade_benefit()[self.product.symbol][0])
        ...
        self.diff_time = time.time() - begin_time + int(self.get_trade_benefit()[self.user][self.market][self.product][0])
        ...
        # for i in range(6, len(self.rows)):
            ...
            try:
                ...
                self.grid_run_time = '网格运行时间:' + str("{0}年{1}月{2}日{3}小时{4}分钟{5}秒".format(
                     ...
                     struct_time.tm_sec))
                print(self.grid_run_time)
                # str(self.http_client_future.get_future_asset(self.product.symbol))
                # tmp = self.http_client_future.get_positionInfo(self.product.symbol)
                # print(f"查看杠杆效果:{tmp}")
                # print("看交易记录：" + str(self.http_client_future.get_my_trades(self.product.symbol)))
                try:
                    self.spot_money = float(self.getAsset())
                ...
                self.gross_profit = str(round(float(self.product.total_earn) / float(self.product.spot_money) * 100, 2)) + '%'
                msg1 = '目前盈利：' + str(self.product.total_earn) + ('(每小时：' + str(round(self.product.total_earn / float(self.diff_time / 3600), 2)) + ')') if float(self.diff_time / 3600) != 0 else ''
                msg2 = '目前网格套利数：' + str(self.product.total_earn_grids) + ', 网格利润率：' + self.gross_profit + ('(每小时：' + str(round(float(self.product.total_earn) / float(self.spot_money) / float(self.diff_time / 3600) * 100, 2)) + '%)') if float(self.diff_time / 3600) != 0 else ''
                msg3 = '网格浮动盈亏, 多单：' + str(sum([(float(self.cur_market_future_price) - float(tmp)) * float(self.quantity) for tmp in self.record_spot_price])) + ', 空单：' + str(sum([(float(tmp) - float(self.cur_market_future_price)) * float(self.quantity) for tmp in self.record_future_price]))
                msg4 = '总仓位数:' + str(self.total_steps) + ', 多仓:' + str(self.spot_step) + ', 空仓:' + str(self.future_step) + ', 底仓：' + str(len(self.long_bottom_position_price))
                msg5 = '仓位具体信息, 多仓:' + str(self.record_spot_price) + ', 空仓:' + str(self.record_future_price) + ', 底仓：' + str(self.long_bottom_position_price) +  '(' + str(self.get_long_bottom_position_scale()) + '), 阈值：' + str(fc.long_bottom_position_share)
                ...
                symbol_to_check_trend = self.product.symbol
                ...
                if fc.position_side == 'LONG':
                    isTrendComing = index.calcTrend_MK(symbol_to_check_trend, "60", ascending, self.demical_length)
                elif fc.position_side == 'SHORT':
                    isTrendComing = index.calcTrend_MK(symbol_to_check_trend, "60", descending, self.demical_length)
                else:
                    isTrendComing = False
                ...
                #清仓操作
                if len(self.record_spot_price) > 0:
                    spot_lost_ratio = (float(self.record_spot_price[0]) - float(self.cur_market_future_price)) / float(self.record_spot_price[0])
                    if spot_lost_ratio > self.cut_position_threshold:
                        msg = '要清掉一份仓位，不然要容易爆仓'
                        print(msg)
                        self.open_spot_price = self.record_spot_price[0]
                        self.close_long(time_format, True)
                        Message.dingding_warn(msg + ', 这份止损了:' + str(self.open_spot_price))
                        del self.record_spot_price[0]
                    else:
                        print('最亏的那份多单损益:' + str(spot_lost_ratio))

                position_delta = min(len(self.record_spot_price) - self.ease_position_share, len(self.record_future_price) - self.ease_position_share)
                if len(self.record_spot_price) == len(self.record_future_price) and len(self.record_spot_price) > self.ease_position_share and len(self.record_future_price) > self.ease_position_share:
                    msg = '减少多空持仓数量，卖掉' + str(self.ease_position_share) + '份'
                    print(msg)
                    for i in range(0, position_delta):
                        self.close_long(time_format, True)
                        Message.dingding_warn('这份多单平掉了:' + str(self.record_spot_price[i]))
                        self.close_short(time_format, True)
                        Message.dingding_warn('这份空单平掉了:' + str(self.record_future_price[i]))
                        del self.record_spot_price[i]
                        del self.record_future_price[i]


                if len(self.record_future_price) > 0:
                    future_lost_ratio = (float(self.cur_market_future_price) - float(self.record_future_price[0])) / float(self.record_future_price[0])
                    if future_lost_ratio > self.cut_position_threshold:
                        msg = '要清掉一份仓位，不然要容易爆仓'
                        print(msg)
                        self.open_future_price = self.record_future_price[0]
                        self.close_short(time_format, True)
                        Message.dingding_warn(msg + ', 这份止损了:' + str(self.open_future_price))
                        del self.record_future_price[0]
                    else:
                ...
                msg9 = "网格大小：多单：" + str(round(self.spot_rising_ratio, 2)) + "%, 空单：" + str(round(self.future_rising_ratio,2)) + "%"
                msg10 = "开平仓数量：" + str(self.quantity)
                print(msg7)
                print(msg8)

                if loop_count % 1800 == 5:#  半小时汇报一次
                    msg = '【' + str(self.product.symbol) + '】' + '汇报脚本运行情况：' + msg1 + ', ' + msg2 + ', ' + msg3 + ', ' + msg4 + ', ' + msg5 + ', ' + msg6 + ', ' + msg7 + ', ' + msg8 + ', ' + msg9 + ', ' + msg10 + self.grid_run_time
                    Message.dingding_warn(msg)

                #设定仓位
                # quantity_basic = (fc.every_time_trade_share if fc.every_time_trade_share else 10.1) / float(self.cur_market_future_price) if self.cur_market_future_price else self.product.quantity
                # self.quantity = self._format(quantity_basic)  # 买的不一定是0.0004,应该是现在的市场价买10u的份额
                # spot_res = None
                # future_res = None

                # if max(float(self.cur_market_future_price), float(self.cur_market_future_price)) < self.product.min_border_price or min(float(self.cur_market_future_price), float(self.cur_market_future_price)) > self.product.max_border_price:
                #     print("市场价超过网格区间上下限啦")
                #     time.sleep(50)
                # el
                # if isTrendComing:
                ...
                if (spot_res is None or not spot_res['id']) and (future_res is None or not future_res['id']):
                    print("这个价格这轮没有买卖成功，开启下一轮")
                    self.save_trade_to_file(time_format, [' ' + time_format, self.cur_market_future_price, "", "", "", ""])
                elif self.crazy_buy and spot_open_long_res and not (future_res and future_res['id']):
                    # 走这里的话，会在同一价位一直买一直买，建议低位时把self.crazy_buy设置为True
                    # 因为狂买模式不要一直触发不然会爆仓，所以还是在建底仓的时候做
                    msg = "进入狂买模式了"
                    Message.dingding_warn(msg)
                    # print("这个价格建仓了，目前仓位列表：" + str(self.long_bottom_position_price) + ", 底仓仓位比例：" + str(self.get_long_bottom_position_scale()))
                else:
                ...
                # 这几个其实有点多余（未验证）
                self.spot_buy_price = self.spot_buy_price
                self.spot_sell_price = self.spot_sell_price
                self.spot_step = self.spot_step

                self.future_buy_price = self.future_buy_price
                self.future_sell_price = self.future_sell_price
                self.future_step = self.future_step
        ...
        print('loop结束了')
        all_invests = sum([float(tmp_spot) for tmp_spot in self.record_spot_price] if len(self.record_spot_price) > 0 else [0]) + sum([float(tmp_future) for tmp_future in self.record_future_price] if len(self.record_future_price) > 0 else [0])
        msg1 = '总结，最终收益：' + str(self.total_earn) + ', 所有仓位数：' + str(self.spot_step + self.future_step) + '， 目前投资额:' + str(float(all_invests) * float(self.quantity)) + '， 曾经最多投资额数：' + str(self.total_invest) + self.grid_run_time
        msg2 = ''
        msg3 = ''
        print(msg1)
        tmp_list2 = [float(tmp) for tmp in self.record_spot_price] if len(self.record_spot_price) > 0 else [0]
        tmp_list_result2 = 0
        for ttt in tmp_list2:
            tmp_list_result2 += ttt
        if len(self.record_spot_price) > 0:
            msg2 = '，多单浮动盈亏：' + str((float(self.cur_market_future_price) - tmp_list_result2 / len(self.record_spot_price)) * float(self.quantity))
            print(msg2)
        tmp_list = [float(tmp) for tmp in self.record_future_price] if len(self.record_future_price) > 0 else [0]
        tmp_list_result = 0
        for ttt in tmp_list:
            tmp_list_result += ttt
        if len(self.record_future_price) > 0:
            msg3 = ', 空单浮动盈亏：' + str((tmp_list_result / len(self.record_future_price) - float(self.cur_market_future_price)) * float(self.quantity))
    ...
    def get_trade_benefit(self):
        ...
        :return:
        '''
        trade_benefit_path = '../data/trade_benefit_multiple.json'
    ...
    def nearly_full_position(self):
        if float(self.total_steps * 100) / (float(self.spot_money) * int(self.leverage)) >= 0.95:
    ...
    def open_long(self, time_format, build_position_share = False):
        ...
        # self.order_list.append(spot_res)
        spot_res = self.http_client_spot.place_order(self.product.symbol, OrderSide.BUY.value, 'LONG', OrderType.MARKET.value, self.quantity, round(float(self.cur_market_future_price), 2))
        ...
        if spot_res and spot_res['id']:
            ...
            self.total_invest += float(self.cur_market_future_price) * float(self.quantity)
            if not build_position_share:
                Message.dingding_warn('【' + str(self.product.symbol) + '】' + str(self.cur_market_future_price) + "买入一份多单了！")
                self.add_record_spot_price(self.cur_market_future_price)
                self.set_spot_share(self.spot_step + 1)
                self.total_steps += 1
            else:
                Message.dingding_warn('【' + str(self.product.symbol) + '】' + str(self.cur_market_future_price) + "【建仓】买入一份多单了！")
            # self.set_spot_ratio()
            # self.set_spot_next_sell_price(float(self.cur_market_future_price))
    ...
    def close_long(self, time_format, cut_position = False, price='none'):
        ...
        if self.spot_step > 0:
            ...
            if order_type == OrderType.LIMIT:
                ...
                return {}
            if spot_res and spot_res['id']:
                self.total_earn += (float(self.cur_market_future_price) - float(
                    self.get_last_spot_price())) * float(self.quantity)
                self.total_earn_grids += 1
                self.addMoney(float(self.cur_market_future_price) * float(self.quantity))
                self.set_spot_share(self.spot_step - 1)
                self.total_steps -= 1
                self.gross_profit = str(round(float(self.total_earn) / float(self.spot_money) * 100, 2)) + '%'
                ...
                print(str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + ', 目前获利：' + str(
                    self.total_earn) + ", 投资总额：" + str(self.total_invest) + ", 多单目前仓位：" + str(
                    self.spot_step))
                self.save_trade_to_file(time_format, [' ' + time_format, self.cur_market_future_price, "",
                ...
                # Message.dingding_warn('【' + str(self.product.symbol) + '】' + str(self.cur_market_future_price) + "平掉一份多单了！")
                earn_this_piece = round((float(self.cur_market_future_price) - float(self.open_spot_price)) * float(self.quantity), 2)
                msg = '【' + str(self.product.symbol) + '】多单卖出获利了！获得：' + str(earn_this_piece) + "， 卖出价格：" + str(self.cur_market_future_price) + "（设定涨：" + str(self.spot_rising_ratio) + "%，实际涨：" + str(round((earn_this_piece / float(self.cur_market_future_price)) * 100, 2)) + "%平仓）" + ", 买入的价格:" + str(self.open_spot_price
                    ) + ", 买入的数量：" + str(self.quantity) + ', 目前总获利：' + str(round(self.total_earn, 2)) + ', 总格子数：' + str(self.total_earn_grids) + ', 利润率：' + self.gross_profit + ', 多仓:' + str(self.spot_step) + ', 空仓:' + str(self.future_step) + ', 仓位具体信息, 多仓:' + str(self.record_spot_price) + ', 空仓:' + str(self.record_future_price) + ', 底仓：' + str(self.long_bottom_position_price) + ', (' + str(self.get_long_bottom_position_scale()) + '), 阈值：' + str(fc.long_bottom_position_share) + ', ' + self.grid_run_time + ', ' + self.last_get_profit_time_delta
                print(msg)
                Message.dingding_warn(msg)
    ...
    def open_short(self, time_format):
        ...
        if self.grid_side == 'LONG':
            future_res['id'] = 'virtual'
            return future_res
        print("进入开空单流程")
        # future_res
        # future_res= {'id': 'Order' + str(random.randint(1000, 10000))}
        # self.order_list.append(future_res)
        future_res = self.http_client_future.place_order(self.product.symbol, OrderSide.SELL.value, "SHORT", OrderType.MARKET.value, self.quantity, round(float(self.cur_market_future_price), 2))

        if future_res and future_res['id']:
            print("开空单成功")
            Message.dingding_warn('【' + str(self.product.symbol) + '】' + str(self.cur_market_future_price) + "买入一份空单了！")
            self.addMoney(float(self.cur_market_future_price) * float(self.quantity))
            self.total_invest += float(self.cur_market_future_price) * float(self.quantity)
    ...
    def close_short(self, time_format, cut_position=False, price='none'):
        ...
        if self.future_step > 0:
            ...
            if price != 'none':
                order_type = OrderType.LIMIT
                time_inforce = "GTC"
            future_res = self.http_client_future.place_order(self.product.symbol, OrderSide.BUY.value, "SHORT", order_type.value, self.quantity, price)
            if order_type == OrderType.LIMIT:
                print('price:' + price + '挂平仓空单了')
                return {}
            if future_res and future_res['id']:
                # Message.dingding_warn('【' + str(self.product.symbol) + '】' + str(self.cur_market_future_price) + "平掉一份空单了！")
                self.decreaseMoney(float(self.cur_market_future_price) * float(self.quantity))
                self.total_earn += (float(self.get_last_future_price()) - float(self.cur_market_future_price)) * float(self.quantity)
                self.total_earn_grids += 1
                self.gross_profit = str(round(float(self.total_earn) / float(self.spot_money) * 100, 2)) + '%'
                if not cut_position:
                ...
                # self.set_future_price(float(self.cur_market_future_price))
                print(str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + ', 目前获利：' + str(
                    self.total_earn) + ", 投资总额：" + str(self.total_invest) + ", 空单目前仓位：" + str(
                    self.future_step))
                self.save_trade_to_file(time_format, [' ' + time_format, self.cur_market_future_price, "", "", "",
                                                      self.cur_market_future_price])
                ...
                msg = '【' + str(self.product.symbol) + '】空单买回获利了！获得：' + str(earn_this_piece) + " usdt， 买回的价格：" + str(self.cur_market_future_price) + "（设定跌:" + str(self.future_rising_ratio) + "%，实际跌:" + str(round((earn_this_piece / float(self.cur_market_future_price)) * 100, 2)) + "%平仓）" + ", 卖出的价格:" + str(
                    self.open_future_price) + ", 买回的数量：" + str(self.quantity) + ', 目前总获利：' + str(round(self.total_earn, 2)) + ', 总格子数：' + str(self.total_earn_grids) + ', 利润率：' + self.gross_profit + ', 多仓:' + str(self.spot_step) + ', 空仓:' + str(self.future_step) + ', 仓位具体信息, 多仓:' + str(self.record_spot_price) + ', 空仓:' + str(self.record_future_price) + ', 底仓：' + str(self.long_bottom_position_price) + ', (' + str(self.get_long_bottom_position_scale()) + '), 阈值：' + str(fc.long_bottom_position_share) + ', ' + self.grid_run_time + ', ' + self.last_get_profit_time_delta
                print(msg)
                Message.dingding_warn(msg)
                self.last_get_profit_time = time.time()
                return future_res
    ...
    def save_trade_to_file(self, time_format, trade_info):
        ...
        #     with open(os.path.join(record_market_price_dir, 'record_market_price_%s_%s.csv' % (time_format.replace(' ', '-'),  self.product.symbol)),
        #               'a+', encoding='utf-8-sig') as ddf:
        #         writer_p = csv.writer(ddf, delimiter=',')
        #         if os.path.getsize(os.path.join(record_market_price_dir,
        #                                         'record_market_price_%s_%s.csv' % (time_format.replace(' ', '-'), self.product.symbol))) == 0:
    ...
    def set_spot_ratio(self):
        ...
        ratio_24hr = round(float(self.http_client_spot.get_ticker_24hour(self.product.symbol)), 1)
        if abs(ratio_24hr) > 6:
                print("24小时上涨或下跌趋势")
                self.spot_rising_ratio = fc.ratio_up_or_down# + self.total_steps / 4
                self.spot_falling_ratio = fc.ratio_up_or_down# + self.total_steps / 4
        else: #震荡时
            print("24小时震荡趋势")
            self.spot_falling_ratio = fc.ratio_no_trendency# + self.total_steps / 8
            self.spot_rising_ratio = fc.ratio_no_trendency# + self.total_steps / 8
        print("24小时涨跌率：ratio_24hr： " + str(ratio_24hr)
              + ", 设置多单上涨的比率：self.spot_rising_ratio:" + str(self.spot_rising_ratio)
              + ", 设置多单下跌的比率：self.spot_falling_ratio:" + str(self.spot_falling_ratio))
    ...
    def set_future_ratio(self):
        ...
        if abs(ratio_24hr) > 6:
                print("24小时上涨或下跌趋势")
                self.future_rising_ratio = fc.ratio_up_or_down / fc.long_buy_ratio_scale# + self.total_steps / 4
                self.future_falling_ratio = fc.ratio_up_or_down / fc.long_buy_ratio_scale# + self.total_steps / 4
        else: #震荡时
            print("24小时震荡趋势")
            self.future_falling_ratio = fc.ratio_no_trendency / fc.long_buy_ratio_scale# + self.total_steps / 8
            self.future_rising_ratio = fc.ratio_no_trendency / fc.long_buy_ratio_scale# + self.total_steps / 8
        print("24小时涨跌率：ratio_24hr： " + str(ratio_24hr)
              + ", 设置空单上涨的比率：self.future_rising_ratio:" + str(self.future_rising_ratio)
              + ", 设置空单下跌的比率：self.future_falling_ratio:" + str(self.future_falling_ratio))
              # + ", 现货的仓位：self.get_spot_share():" + str(self.get_spot_share())
              # + ", 合约的仓位：self.get_future_share():" + str(self.get_future_share()))

    def add_record_spot_price(self, value):
        self.record_spot_price.append(value)
        self.record_spot_price.sort(reverse=True)#降序
        print('record_spot_price:' + str(self.record_spot_price))
        self.save_trade_info()

    def get_last_spot_price(self):
        if len(self.record_spot_price) == 0:
            return self.spot_buy_price
        return self.record_spot_price[-1]

    def get_last_spot_prices(self):
        return self.record_spot_price

    def get_last_future_prices(self):
        return self.record_future_price

    def remove_last_spot_price(self):
        del self.record_spot_price[-1]
        self.save_trade_info()

    def add_record_future_price(self, value):
        self.record_future_price.append(value)
        self.record_future_price.sort()
        print('record_future_price:' + str(self.record_future_price))
        self.save_trade_info()

    def get_last_future_price(self):
        if len(self.record_future_price) == 0:
            return self.future_sell_price
        # else:
        #     print('sdfdfg:' + str(self.future_step) + str(self.record_future_price))
        return self.record_future_price[-1]

    def remove_last_future_price(self):
        del self.record_future_price[-1]
        self.save_trade_info()

    def get_spot_share(self):
        # tmp = self.http_client.get_positionInfo(self.product.symbol)
        # for item in tmp:  # 多头持仓均价
        #     if item['positionSide'] == "LONG":
        #         self.spot_step = float(item['positionAmt'])
        #         break
        print("get_spot_share")
        #觉得应该去掉文件中没有记录价格的那部分仓位
        self.spot_step = int(len(self.record_spot_price)) #int(float(self.http_client_spot.get_spot_position_info(self.product.coin)) / float(self.quantity)) # 取该货币的仓位，处以每份的数量，结果为份数
        #测试时默认是0开始
        self.spot_step = self.spot_step
        print("get_spot_share, self.spot_step:" + str(self.spot_step), ", position:" + self.http_client_spot.get_future_position_info(self.product.symbol) + ", every piece quantity:" + self.quantity)
    ...
    def set_spot_next_buy_price(self, deal_price):
        ...
        # self.spot_buy_price = round(min(float(self.record_future_price[-1]) if len(self.record_future_price) > 0 and self.grid_side == 'BOTH' else 999999, deal_price * (1 - self.spot_falling_ratio / 100)), demical_point)  # 多单跌的时候补仓 # 保留2位小数
        self.spot_buy_price = round(deal_price * (1 - self.spot_falling_ratio / 100), demical_point)  # 多单跌的时候补仓 # 保留2位小数
        self.spot_buy_price = self.spot_buy_price
    ...
    def set_spot_next_sell_price(self, deal_price):
        price_str_list = str(deal_price).split(".")
        demical_point = len(price_str_list[1]) if len(price_str_list) > 1 else 0 + 2
        self.spot_sell_price = round(deal_price * (1 + self.spot_rising_ratio / 100), demical_point)
        self.spot_sell_price = max(self.spot_sell_price, float(self.record_spot_price[-1]) if len(self.record_spot_price) > 0 else 0) # 如果多单平仓价比列表里最小的还小，那交易时就亏本了啊
        # print("设置接下来多单卖出的价格:" + str(self.spot_sell_price))

    def set_future_next_sell_price(self, deal_price):
        price_str_list = str(deal_price).split(".")
        demical_point = len(price_str_list[1]) if len(price_str_list) > 1 else 0 + 2
        self.future_sell_price = round(deal_price * (1 + self.future_falling_ratio / 100), demical_point)
        self.future_sell_price = self.future_sell_price
    ...
    def set_future_next_buy_price(self, deal_price):
        price_str_list = str(deal_price).split(".")
        demical_point = len(price_str_list[1]) if len(price_str_list) > 1 else 0 + 2
        self.future_buy_price = round(deal_price * (1 - self.future_rising_ratio / 100), demical_point)  #空单涨的时候补仓 # 保留2位小数
        self.future_buy_price = min(self.future_buy_price, float(self.record_future_price[-1]) if len(self.record_future_price) > 0 else 999999) # 如果空单平仓价比列表里最大的还大，那交易时就亏本了啊
        # print("设置接下来空单的买回价格:" + str(self.future_buy_price))

    def adjust_prices(self):
        # if self.grid_side == 'BOTH':
        #     min_buy_price = min(self.spot_buy_price, self.future_buy_price)
        #     max_sell_price = max(self.spot_sell_price, self.future_sell_price)
        #     self.spot_buy_price = min_buy_price
        #     self.spot_sell_price = max_sell_price
        #     self.future_sell_price = max_sell_price
        #     self.future_buy_price = min_buy_price
        #     self.spot_buy_price = self.spot_buy_price
        #     self.spot_sell_price = self.spot_sell_price
        #     self.future_sell_price = self.future_sell_price
        #     self.future_buy_price = self.future_buy_price
    ...
    def place_left_orders(self):
        ...
        print('停止时，自动挂单吧')
        time_format = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        for spot_price in self.record_spot_price:
            self.close_long(time_format, False, str(int(float(spot_price) * (1 + self.spot_rising_ratio / 100))))
        for future_price in self.record_future_price:
            self.close_short(time_format, False, str(int(float(future_price) * (1 - self.future_falling_ratio / 100))))

    def get_future_share(self):
        # self.future_step = len(self.record_future_price)
        # self.future_step = self.future_step
        ...
        #         self.future_step = len(self.record_future_price) #abs(int(float(item['positionAmt']) / float(self.quantity)))
        #         self.future_step = self.future_step
    ...
    def init_record_price_list(self):
        ...
        # orders = self.http_client_spot.get_all_orders(self.product.symbol)
        # for order in orders:
        #     if order.get('side') == OrderSide.BUY.value and order.get('status') == OrderStatus.FILLED.value:
        #         self.record_spot_price.append(order.get('price'))

        with open('../data/trade_info_multiple.json', 'r') as df:
            if os.path.getsize(df.name) == 0:
                return
            trade_info_all = json.load(df)
            record_price_dict_to_file = trade_info_all[self.user][self.market][self.product]
            if 'record_spot_price' in record_price_dict_to_file.keys():
                ...
                self.record_spot_price = record_price_dict_to_file['record_spot_price']
            if 'record_future_price' in record_price_dict_to_file.keys():
                ...
                self.record_future_price = record_price_dict_to_file['record_future_price']
            if 'long_bottom_position_price' in record_price_dict_to_file.keys():
                ...
                self.long_bottom_position_price = record_price_dict_to_file['long_bottom_position_price']
            self.total_steps = len(self.record_spot_price) + len(self.record_future_price) + len(self.long_bottom_position_price)

    # def init_record_future_price_list(self):
    #     orders = self.http_client_future.get_all_orders(self.product.symbol)
    #     for order in orders:
    #         if order.get('positionSide') == 'SHORT' and order.get('side') == OrderSide.SELL.value and order.get('status') == OrderStatus.FILLED.value:
    #             self.record_future_price.append(order.get('price'))

    def save_trade_info(self):
        print("存储记录的价格到文件里")
        print(f"save_trade_info, record_spot_price:{self.record_spot_price}, record_future_price:{self.record_future_price}, self.long_bottom_position_price:{self.long_bottom_position_price}")
        record_price_file_path = '../data/trade_info_multiple.json'
        if os.path.getsize(record_price_file_path) == 0:
            record_price_dict_to_file = {
                self.user:{
                    self.market:{
                        self.product.symbol: {'record_spot_price': self.record_spot_price, 'record_future_price': self.record_future_price, 'long_bottom_position_price': self.long_bottom_position_price}}}}
        else:
            with open(record_price_file_path, "r") as df:
                record_price_dict_to_file = json.load(df)
            if not record_price_dict_to_file or not record_price_dict_to_file[self.user]:
                record_price_dict_to_file = {
                    self.user: {
                        self.market: {
                            self.product.symbol: {'record_spot_price': self.record_spot_price,
                                                  'record_future_price': self.record_future_price,
                                                  'long_bottom_position_price': self.long_bottom_position_price}}}}

        with open('../data/trade_info_multiple.json', "w") as df:
    ...
    def save_trade_benefit(self):
        ...
        :return:
        '''
        record_benefit_list_to_file = [self.diff_time, str(self.total_earn), self.gross_profit, str(self.total_earn_grids), str(self.total_steps), str(self.future_step), str(len(self.long_bottom_position_price))]
        trade_benefit_path = '../data/trade_benefit_FTX.json'
        with open(trade_benefit_path, "r") as df_read:
            load_dict = json.load(df_read)
            print(load_dict)
            if load_dict and self.product.symbol in load_dict.keys():
                load_dict[self.product.symbol] = record_benefit_list_to_file
    ...
    def close_previous_position(self, time_format):
        ...
        ret_list_spot = []
        ret_list_future = []
        for tmp in self.record_spot_price:
            self.cur_market_future_price = self.http_client_spot.get_latest_price(self.product.symbol)
            if float(tmp) <= float(self.cur_market_future_price):
                self.close_long(time_format, True)
            else:
                ret_list_spot.append(tmp)
        self.record_spot_price = ret_list_spot
        # print("ssdfsdf:" + str(self.record_spot_price))

        for tmp in self.record_future_price:
            self.cur_market_future_price = self.http_client_spot.get_latest_price(self.product.symbol)
            if float(tmp) >= float(self.cur_market_future_price):
                self.close_short(time_format, True)
            else:
                ret_list_future.append(tmp)
        self.record_future_price = ret_list_future
        # del ret_list_spot[:]
        # del ret_list_future[:]
    ...
    def long_bottom_position_full(self):
        ...
        :return:
        '''
        tmp_list = [float(tmp) for tmp in self.long_bottom_position_price]
    ...
    def get_long_bottom_position_scale(self):
        current_position_share = (sum([float(tmp) for tmp in self.long_bottom_position_price]) * float(
    ...
    def build_long_bottom_position(self, price, time_format):
        print('当前的价格' + str(self.cur_market_future_price) + '，目前的底仓列表：' + str(self.long_bottom_position_price))
        #todo need improve
        len_position_share = len(self.long_bottom_position_price)
        ...
        if not self.long_bottom_position_full():
            ...
            # retain_list = []
            tmp = max(self.long_bottom_position_price)
            if float(tmp) > float(price):
                print('这个价格超过目前的底仓列表，剔除【' + str(tmp) + '】，将它挂单')
                self.close_long(time_format, False, str(int(float(tmp) * (1 + self.spot_rising_ratio / 100))))#挂掉出了
                tick_out_price = tmp
                need_open_long_bottom_position = True
        if need_open_long_bottom_position and tick_out_price:
            self.long_bottom_position_price.remove(tick_out_price)#把价格大的剔除
            self.save_trade_info()

        if need_open_long_bottom_position:
            print('这个价格需要加入底仓')
            self.long_bottom_position_price.append(price)
            self.save_trade_info()
            need_open_long_bottom_position = False
            return self.open_long(time_format, True)#加入底仓后，需要开单，才算真正加入了底仓


def open_receiver(self, number=0):
    #todo 最好还是放在另外一个进程里，方便命令调起网格策略
    app.run(host='104.225.143.245', port=5000 + number, threaded=True)
...
def normal_exit(self):
        while not fc.stop_singal_from_client:
            ...
            # current_falling_ratio = self.spot_falling_ratio
            #     current_rising_ratio = self.rising_ratio
            #     self.set_ratio()
...
def dynamicSettings(hengedGrid, fc):
    if not fc.stop_singal_from_client:
        if fc.change_ratio_singal_from_client:
            # print(str(fc.stop_singal_from_client))
            # if fc.change_ratio_singal_from_client:
            hengedGrid.set_ratio_and_price()
            fc.change_ratio_singal_from_client = False
        if fc.change_every_time_trade_share_signal_from_client:
            hengedGrid.cur_market_future_price = hengedGrid.http_client_spot.get_latest_price(
                hengedGrid.product.symbol)  # self.http_client_future.get_latest_price(self.product.symbol).get('price')
            quantity_basic = round(
                (fc.every_time_trade_share / float(hengedGrid.cur_market_future_price)), 3)
            hengedGrid.quantity = hengedGrid.format(quantity_basic)
            fc.change_every_time_trade_share_signal_from_client = False
        if fc.change_quantity_singal_from_client:
            hengedGrid.quantity = str(fc.quantity)
            fc.change_quantity_singal_from_client = False
        if fc.change_position_side_singal_from_client:
            hengedGrid.grid_side = fc.position_side
            fc.change_position_side_singal_from_client = False
        if fc.change_long_bottom_position_share_singal_from_client:
            hengedGrid.long_bottom_position_share = fc.long_bottom_position_share
            fc.change_long_bottom_position_share_singal_from_client = False
        if fc.cut_position_threshold_singal_from_client:
            hengedGrid.cut_position_threshold = fc.cut_position_threshold
            fc.cut_position_threshold_singal_from_client = False
        if fc.ease_position_share_singal_from_client:
            hengedGrid.ease_position_share = fc.ease_position_share
            fc.ease_position_share_singal_from_client = False
        if fc.update_position_list_signal_from_client:
            hengedGrid.init_record_price_list()
            fc.update_position_list_signal_from_client = False
        if fc.long_buy_ratio_scale_signal_from_client:
            hengedGrid.long_buy_ratio_scale = fc.long_buy_ratio_scale
            fc.long_buy_ratio_scale_signal_from_client = False
        if fc.open_trend_trade_singal_from_client:
            hengedGrid.open_trend_trade = fc.open_trend_trade
            fc.open_trend_trade_singal_from_client = False
    elif fc.stop_singal_from_client:
        run_thread.join()

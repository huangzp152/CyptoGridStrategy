...
class CalcIndex:
    ...
    def __init__(self, market, product, test_data=None):
        self.coinType = product.symbol  # 交易币种
        if market.platform == "binance":
            self.http_client = BinanceFutureHttp(api_key=market.api_key, secret=market.api_secret, proxy_host='', proxy_port=0)
        elif market.platform == "ftx":
            self.http_client = ftx_future(api_key=market.api_key, api_secret=market.api_secret)

...
class flaskConfig(object):
    def __init__(self):
        ...
        self.open_trend_trade_singal_from_client=False
        self.change_position_side_singal_from_client=False
        self.ratio_no_trendency=0.8
        self.ratio_up_or_down=1.6
        self.long_buy_ratio_scale=0.33 # 0.25 多空格子利率的比例，0.25即1:4,就是比如做多网格的利率是0.3的话，做空就是0.3 / 0.25= 1.2,通过多空格子大小的差异来形成对冲获利
        self.every_time_trade_share = 150 # 33 测试环境下要求小数点后面3位精度，买10u的话只要0.000304左右，四舍五入就是0.000了，这样买不上
...
@app.route(f'/grid/{config.symbol}/change/every_time_trade_share', methods=['GET'])
...
@app.route(f'/grid/{config.symbol}/change/quantity', methods=['GET'])
...
@app.route(f'/grid/{config.symbol}/change/long_bottom_position_share', methods=['GET'])
...
@app.route(f'/grid/{config.symbol}/change/position_side', methods=['GET'])
...
@app.route(f'/grid/{config.symbol}/change/leverage', methods=['GET'])
...
@app.route(f'/grid/{config.symbol}/change/cut_position_threshold', methods=['GET'])
...
@app.route(f'/grid/{config.symbol}/change/ease_position_share', methods=['GET'])
...
@app.route(f'/grid/{config.symbol}/change/long_buy_ratio_scale', methods=['GET'])
...
@app.route(f'/grid/{config.symbol}/change/crazy_buy', methods=['GET'])
def crazy_buy():
    try:
        fc.crazy_buy = True
    except RuntimeError as e:
...
@app.route(f'/grid/{config.symbol}/change/open_trend_trade', methods=['GET'])
...
@app.route(f'/grid/{config.symbol}/start', methods=['GET'])
def grid_start():
    try:
        fc.start_grid = True
    except RuntimeError as e:
